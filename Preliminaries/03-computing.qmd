...
We call `<-` the "**storage arrow**." Think of it as stowing the right-hand value in a box labelled with the name on the left-hand side. 
...
Whenever you refresh or re-open a document, the active R chunks will all be in a not-yet-run state. You have to run the code individually for each chunk you want to use or which are used by other chunks. By and large, we write each R chunks to avoid the need to run earlier chunks ... but not always!
