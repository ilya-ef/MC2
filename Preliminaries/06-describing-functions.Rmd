...
:--:|:--:|:--:
![](www/pb-recip-gg.png)| ![](www/pb-sin-gg.png) | ![](www/pb-exp-gg.png) 
...
Of the pattern-book functions in @fig-three-for-illustration: the exponential and its intimate companion---the logarithm function are monotonic: the exponential grows monotonically as does the logarithm. The sinusoid is not monotonic over any domain longer than half a cycle: the function switches between positive slope and negative slope in different parts of the cycle (as is evident in @fig-slope-at-point2). 
...
Several of the pattern-book functions have horizontal or vertical asymptotes or both. For instance, the reciprocal function ($x^{-1}$), in addition to the vertical asymptote described in the previous paragraph, has a horizontal asymptote of zero for both $x \rightarrow \infty$ and $x \rightarrow -\infty$.
