...
In many STEM fields, worker use special, usually named quantities that are dimensionless. Being dimensionless is different from having no units. For instance, an angle is a dimensionless quantity (arc length divided by radius length) but is stated in units. For instance, 180 degrees is the same as $\pi$ radians. (More obscurely, some fields use "grads." A grad is one-hundredth of a right angle, that is, a little less than a degree.) Many people have heard of the  "[*mach*](https://en.wikipedia.org/wiki/Mach_number)" and that 1 mach corresponds to the speed of sound. But speed has dimension---L/T, as in meters per second or miles per hour. And *mach* is a dimensionless quantity. How is *mach* constructed to be dimensionless and yet still be about speed? 
